...
Below `groupby`was mapped to a combination of the colums Sex and Country:
...
## 2.2 Examples for aggregation of numeric data 
...
Default outliers are flagged when outside the range from "25th" percentile - 1.5 x IQR to "75th" percentiles + 1.5 x IQR, as suggested by McGill and implemented by the current boxplot functions from ggplot (`geom_boxplot`).
...
In the following example the aggregation is customized to set the whiskers to 10% and 90% and the outlier range is customized to show all points outside the whiskers.
For that we define a function `myStatFunOutlier` which has as input the vector of the values, and as output the vector of values outside the outlier range. If no value is outside range NA is returned.
