...
For convenience, `setDefaultTheme()` and `resetDefaultTheme()` set or restore the `ospsuite.plots` theme as the global `ggplot2` theme for the whole session. `setDefaultTheme()` simply calls `theme_set(themeOspsuite())` and is also called by `setDefaults()`.
