...
* simData2Dimension: dataset wehre the "values" column has mixed diemnsions: concentration and fraction, 
...
Also available is error_relative where a multiplicative error is assumed: $ymin = values*error\_relative$, $ymax = values\error\_relative$
...
A similar plot can produces by combining caption and gender wh interaction
...
The column header must correspondent to one ov the aesthetics defined in `groupgroupAesthetics`.
