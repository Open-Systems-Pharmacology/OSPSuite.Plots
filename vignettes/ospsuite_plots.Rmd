...
The library is based on `ggplot2` functionality. It uses also the library `ggh4x`.
...
for customized functions add `scale_shape_manual(values = getOspsuite.plots.option(optionKey = OptionKeys$ospsuite.plots.shapeValues))`
...
The names of all options defined by this package start with the package name `ospsuite.plots` as prefix and a suffix. The suffixes are listed in the enumeration `OptionKeys`
There exist two helper functions (`setOspsuite.plost.option` and `getOspsuite.plost.option`) to set and get these options 
...
* switch watermark on and off (option key = `watermark_enabled`, default = `TRUE`) 
* select the label (option key = `watermark_label`, default =  "preliminary analysis") 
* customize format (option key = `watermark_format`, default =  list(
    x = 0.5,
    y = 0.5,
    color = "grey20",
    angle = 30,
...
All plot functions have input variable `geom*Attribute` which are passed as variables to the corresponding ggplot layer.
The defaults of the input variables can be set by options
...
df <- rbind(
  ...
  data.frame(
    ...
    functions = c("plotTimeProfile()", "plotResVsCov()", "plotRatioVsCov()", "plotPredVsObs()")
  ),
  data.frame(
  ...
  tidyr::pivot_wider(names_from = functions, values_from = inputVariable, values_fill = " ")
...
* `BoxplotAttributes = list(position = position_dodge(width = 1),
      color = "black")`   
* `HistAttributes = list(
      bins = 10,
...
# alpha of LLOQ values  c("TRUE" = 0.3, "FALSE" = 1)
getOspsuite.plots.option(optionKey = "LLOQAlphaVector")

# linetype LLOQ comparison lines  'dashed'
...
```{r, eval = FALSE}
# percentiles for boxwhisker plots c(0.05, 0.25, 0.5, 0.75, 0.95)
getOspsuite.plots.option(optionKey = "Percentiles")
```


## 2.5.4 Options to define export format 

There are options to define the export format using function `exportPlot` (see details below)
...
The letter options are used directly as input for `ggplot2::ggsave` so check help for available values.
...
Exporting the Plot:
Using the `exportPlot` function, you can easily save this plot to a file:
...
In this case, you don't need to specify the width and height explicitly; the function calculates them for you.
As default width the value saved in the `ospsuite.plots` option export.width is used.
If a aspect.ratio is defined in the theme of the plot, Height will adjusted accordingly, otherwise the function exports square figures.


# 6. Shapes

## 6.1 Default shapes ggplot:
