...
`setDefaults()` sets the theme, discrete color palette, shapes and various options. All objects can als be set separately as described below.
...
The example below show a plot with  
...
* D customize plot: set grey scale for this plot only using function `scale_fill_grey()`    
...
* F customized plots set grey scale for this plot only using function `ggsci::scale_color_lancet()`    
...
see `vignette("PK-Ratio Plots", package = "ospsuite.plots")`
...
All function where this aesthtic is used have also a variable `groupAesthtics. The mapping `groupby`is copied to all aesthetics listed within this variable and to aesthetic `group`.  
...
For all other functions residuals ar calculated if both aesthetics are mapped. 
...
```{r additional-aesthtic-table, echo=FALSE}
df <- rbind(
  data.frame(
    aesthetic = "`groupby`",
    functions = c(
