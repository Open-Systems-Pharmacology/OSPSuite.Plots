...
To calculate this, the function `getCountsWithin` is called internally without stratification (variable `groups = NULL`). If you want to calculate the number of ratios with stratification, you can call `getCountsWithin` and define the stratification with the variable `groups`.

```{r qualification-tables, results='asis'}
countsWithinSex <- getCountsWithin(
  data = pkRatioData,
  yColumn = "Ratio",
  groups = c("Sex")
)

knitr::kable(countsWithinSex, caption = "Ratios stratified by gender")
```
