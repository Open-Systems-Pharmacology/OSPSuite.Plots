...
OSP shape ordering is controlled per plot, not via a global option. Use `geom_point_osp()` together with `scale_shape_osp()` for automatic assignment from `ospShapeNames`, `scale_shape_osp_manual(values = c(level1 = "circle", level2 = "diamond", ...))` for an explicit mapping, or `scale_shape_osp_identity()` when the data already contains shape names. Raw `ggplot2::geom_point()` keeps using ggplot2's native pch shapes.
