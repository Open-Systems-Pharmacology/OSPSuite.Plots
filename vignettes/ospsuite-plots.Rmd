...
## 2.5.4 Options for Box-Whisker Percentiles

The default percentiles used by `plotBoxWhisker()` are stored as an option and can be changed globally:

- `percentiles`: A numeric vector of five quantiles defining the lower whisker, lower box, median, upper box, and upper whisker (default = `c(0.05, 0.25, 0.5, 0.75, 0.95)`).

```{r default-percentiles, eval=FALSE}
# Get current default percentiles
getOspsuite.plots.option(optionKey = OptionKeys$percentiles)

# Change default percentiles globally
...
The `geomPointUnicode` option controls whether `ospsuite.plots` functions use the Unicode-aware `geomPointUnicode()` geom instead of the standard `geom_point()` when plotting points. This is set internally when calling `setDefaults(pointAsUnicode = TRUE/FALSE)`.

- `geomPointUnicode`: Logical, default `FALSE`. When `TRUE`, Unicode symbols are used for plot points (requires the `{showtext}` package; see Section 6.2).

```{r geom-point-unicode-option, eval=FALSE}
# Check current mode
