...
A geom that renders OSP shapes using grid primitives. Uses shape names
from `ospShapeNames`. Automatically applies
[`scale_shape_osp()`](https://www.open-systems-pharmacology.org/OSPSuite.Plots/dev/reference/scale_shape_osp.md)
when added to a plot (unless a shape scale is already present).
