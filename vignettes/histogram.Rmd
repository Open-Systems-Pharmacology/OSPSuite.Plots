...
That changes the position, but attention, the entry `bins = 10`is now overwritten and the default ggplot number of 30 is used.
...
### 2.1.3 Basic example: change of psoition but keep number of bins
...
If some of the data should be omitted, we could do this by mapping a logcial to the aesthetic `mdv`. Below we excluded data above the value of 4: 
...
### 2.1.6 Startified by a combination of columns
...
### 2.1.7 Usage of bin boundaries

`geom_histogram` provides many possibilities to customize the binning. Below we defined the bin boundaries 
by adding the entry `breaks` to `geomHistAttributes`. See `?geom_histogram`
...
For the most common distributons the key normal (instead of norm) and lognormal (instead of `lnorm`) are also accepted.  


The vertical line indicates the mean. The function to calculate the mean is determined by the variable `meanFunction`.   
Available are  
...
As fit is based on binning, and binning is depended on scale, a log scale has to be set before the distribution fit. Please use variable `xscale = 'log'`
