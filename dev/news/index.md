...
- On log-scaled time profile plots, negative values in y-mapped
  aesthetics (`y`, `ymin`, `ymax`, etc.) are now replaced with `0`
  instead of `NA`. `log(0) = -Inf` is kept at the axis minimum by
  ggplot’s default out-of-bounds handling
